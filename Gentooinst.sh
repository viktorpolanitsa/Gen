...
# -----------------------------
REQUIRED_UTILS=("equery" "grep" "awk" "sed" "ls" "cp" "mkdir" "rm" "ln" "make" "grub-mkconfig")
for util in "${REQUIRED_UTILS[@]}"; do
    if ! command -v "$util" &>/dev/null; then
        echo "[ERROR] Required utility '$util' not found!"
        echo "Please install it first: emerge --ask sys-apps/${util}"
        exit 1
    fi
done
...
mapfile -t KERNELS < <(find /usr/src -maxdepth 1 -type d -name "linux-*" -exec basename {} \;)
