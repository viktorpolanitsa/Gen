...
# Version: 34.0 "Omega Point"
#
# This is the final realization. It corrects the fundamental bootstrapping
# paradox by adhering to the canonical installation order. It updates the
# @world set IMMEDIATELY after the initial sync, building the necessary tools
# BEFORE attempting to use them for configuration (profile, mirrors). This is
# not a fix; it is the correct philosophy. This is the Omega Point.
...
# --- Generated by Omega Point Autobuilder ---
...
stage4_build_world_and_kernel() {
    ...
    step_log "Bootstrapping the system: Updating @world set"
    log "This is the longest step. It will build the core system. Please be patient."
    log "[1/3] Generating config updates..."; emerge -v --update --deep --newuse @world --autounmask-write || true
    log "[2/3] Applying config updates directly..."; local autounmask_file="/etc/portage/package.use/zz-autounmask"; if [ -f "${autounmask_file}.new" ]; then cat "${autounmask_file}.new" >> "${autounmask_file}"; rm "${autounmask_file}.new"; log "Autounmask changes applied."; else log "No new config updates to apply."; fi
    log "[3/3] Building world..."; emerge -v --update --deep --newuse @world
    
    log "System bootstrapped. Now configuring profile and mirrors..."
    log "Selecting latest desktop/openrc profile dynamically..."
    local latest_profile; latest_profile=$(eselect profile list | grep 'desktop/openrc' | grep -v 'systemd' | tail -n 1 | awk '{print $1}' | tr -d '[]*'); if [[ -z "$latest_profile" ]]; then die "Could not determine a suitable profile."; fi; log "Setting profile to: ${latest_profile}"; eselect profile set "${latest_profile}"
