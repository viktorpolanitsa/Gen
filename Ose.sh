...
# Version: 30.0 "Oracle"
#
# This is the final revelation. It corrects the fundamental flaw of hardcoding
# a profile path that becomes obsolete over time. It restores the dynamic
# profile selection mechanism, hardened and made robust, ensuring the script
# adapts to the present state of the synced Portage tree. It does not rely on
# the past. It sees what IS, and acts. This is the Oracle.
...
# --- Generated by Oracle Autobuilder ---
...
stage3_configure_in_chroot() {
    ...
    latest_profile=$(eselect profile list | grep 'desktop/openrc' | grep -v 'systemd' | tail -n 1 | awk '{print $1}' | tr -d '[]*')
    if [[ -z "$latest_profile" ]]; then die "Could not determine a suitable profile."; fi
    log "Setting profile to: ${latest_profile}"
