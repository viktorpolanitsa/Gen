...
# Version: 31.0 "Void"
#
# This is the final philosophy. It achieves ultimate resilience by removing the
# final dependency on the transient, unpredictable state of the mirror network.
# The fragile `emerge --sync` command is removed from the initial setup. The
# self-consistent snapshot from `emerge-webrsync` is sufficient to build the
# system. By removing this point of failure, we embrace the void and achieve
# perfection through subtraction, not addition.
...
stage3_configure_in_chroot() {
    step_log "System Configuration (Inside Chroot)"; source /etc/profile; export PS1="(chroot) ${PS1:-}"
    
    # <<< THE HEALING / ИСЦЕЛЕНИЕ >>>
    log "Syncing Portage tree snapshot..."; emerge-webrsync || die "Failed to sync portage tree snapshot."
    # The fragile `emerge --sync` is removed. The snapshot is enough.
