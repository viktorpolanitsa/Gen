...
# Version: 10.3.9 "The Interpreter"
#
# Changelog:
# - v10.3.9:
#   - BUGFIX: Fixed `syntax error near unexpected token 'y'`. Replaced bash-specific regex
#     in `ask_confirm` with a POSIX-compliant `case` statement, making it robust against
#     execution via `sh`.
#   - ROBUSTNESS: Forced script execution via `/bin/bash` inside the chroot to prevent
#     shebang interpretation issues and guarantee the correct shell.
#   - REFACTOR: Simplified the main stage-calling loop for better readability and reliability.
#   - REFACTOR: Cleaned up GRUB platform detection logic in `interactive_setup`.
# - v10.3.8: Hardened security, improved hardware detection, and added user-safety checks.
...
# ### ИСПРАВЛЕНО: `ask_confirm` переписана на POSIX-совместимый синтаксис.
# Это исправляет ошибку "syntax error near unexpected token 'y'".
ask_confirm() {
    if ${FORCE_MODE:-false}; then return 0; fi
...
# ==============================================================================
interactive_setup() {
    # ... (содержимое функции в основном без изменений, пропущено для краткости) ...
    # ### ИСПРАВЛЕНО: Упрощена и сделана более читаемой логика записи в файл конфигурации
    local grub_platform="pc"
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        grub_platform="efi-64"
    fi
    
    # ... (в конце функции `interactive_setup`)
...
stage2_prepare_chroot() {
    # ... (содержимое функции без изменений, пропущено для краткости) ...
    # ### ИСПРАВЛЕНО: Принудительный вызов /bin/bash в chroot.
    # Это гарантирует, что скрипт будет выполняться правильным интерпретатором,
    # решая фундаментальную причину многих потенциальных ошибок.
...
main() {
    ...
    if [[ "${1:-}" == "--chrooted" ]]; then
        ...
        # ### ИСПРАВЛЕНО: Простой и понятный цикл вызова стадий
        declare -a stages=(
            self_check
            pre_flight_checks
            ensure_dependencies
        ...
        for i in "${!stages[@]}"; do
            ...
            if (( START_STAGE <= stage_num )); then
                "$stage_func"
                # Сохраняем чекпоинт после каждой стадии до chroot
