...
# --- Generated by Nexus Prime Autobuilder (AMD APU Optimized) ---
...
# ==============================================================================
# --- STAGES 3 through 7 (No changes here) ---
# ==============================================================================
...
stage4_build_world_and_kernel() {
    ...
    log "[3/4] Building world with initial settings..."; emerge -v --update --deep --newuse @world
    log "Optimizing for CPU architecture..."; emerge -v app-portage/cpuid2cpuflags
    local cpu_flags_x86; cpu_flags_x86=$(cpuid2cpuflags)
    if [[ -n "$cpu_flags_x86" ]]; then
        log "Adding CPU_FLAGS_X86=\"${cpu_flags_x86}\" to make.conf"
        echo "CPU_FLAGS_X86=\"${cpu_flags_x86}\"" >> /etc/portage/make.conf
    else
        warn "Could not determine CPU_FLAGS_X86."
    fi
