...
# Version: 4.0 "Initiator"
#
# This script is fully autonomous. It detects whether it is running outside
# or inside a chroot environment. If outside, it prepares the chroot,
# copies itself inside, and re-executes. If inside, it performs the full
...
GENTOO_MNT="/mnt/gentoo" # The mount point of your new Gentoo system
...
# ==============================================================================
# --- CHROOT PREPARATION LOGIC (RUNS OUTSIDE) ---
# ==============================================================================
prepare_and_enter_chroot() {
    log "--- Stage 1: Chroot Preparation ---"
    
    if [[ "$(id -u)" -ne 0 ]]; then
        die "This script must be run as root."
    fi

    log "Checking mount point at ${GENTOO_MNT}..."
    if [[ ! -d "${GENTOO_MNT}/etc" ]]; then
        die "Mount point ${GENTOO_MNT} does not look like a Gentoo system. Aborting."
    fi

    log "Mounting virtual filesystems..."
    mount --types proc /proc "${GENTOO_MNT}/proc"
    mount --rbind /sys "${GENTOO_MNT}/sys"
    mount --make-rslave "${GENTOO_MNT}/sys"
    mount --rbind /dev "${GENTOO_MNT}/dev"
    mount --make-rslave "${GENTOO_MNT}/dev"

    log "Copying DNS info..."
    cp --dereference /etc/resolv.conf "${GENTOO_MNT}/etc/"

    local script_name
    script_name=$(basename "$0")
    local script_dest_path="${GENTOO_MNT}/root/${script_name}"
    log "Copying this script into the chroot at ${script_dest_path}..."
    cp "$0" "${script_dest_path}"
    chmod +x "${script_dest_path}"

    log "Entering chroot and executing Stage 2..."
    # Pass all original arguments to the script inside the chroot
    chroot "${GENTOO_MNT}" "/root/${script_name}" --chrooted "$@"
    
    log "Chroot execution finished. Cleaning up..."
    rm -f "${script_dest_path}"
...
# ==============================================================================
# --- MAIN INSTALLATION LOGIC (RUNS INSIDE CHROOT) ---
# ==============================================================================
run_installation_logic() {
    # All functions from the "Praetorian" script are defined here
    ...
    load_config() {
      ...
      if [[ ! -f "$CONFIG_FILE" ]]; then
        ...
        printf '%s\n' \
          '# Gentoo Autobuilder Configuration ("Initiator" version)' '' \
          '# --- Hardware Settings ---' 'CPU_MARCH="native"' \
          'VIDEO_CARDS="amdgpu radeonsi radeon"' 'INPUT_DEVICES="libinput"' '' \
          '# --- Localization ---' 'SYSTEM_LOCALE="ru_RU.UTF-8"' \
          'SYSTEM_TIMEZONE="Europe/Moscow"' '' \
          '# --- Emerge Settings ---' 'EMERGE_JOBS="$(nproc --all 2>/dev/null || echo 4)"' > "$CONFIG_FILE"
        log "Default configuration created at $CONFIG_FILE. Please review it and run the script again."
        exit 0
      fi
      source "$CONFIG_FILE" || die "Failed to load configuration from $CONFIG_FILE."
    }
    ...
    preflight_checks() {
        ...
        if command -v on_ac_power >/dev/null && ! on_ac_power; then
            if ! $FORCE_MODE; then die "System is on battery power. Unsafe. Aborting. (Use --force to override)"; else warn "Running on battery! Proceeding due to --force."; fi
        fi
    ...
    final_summary() { log "--- SCRIPT COMPLETE ---"; echo -e "\n\n${C_GREEN}Gentoo Ultimate Autobuilder 'Initiator' has finished.${C_RESET}"; echo "Summary of actions:"; echo "- System localized, configured, and tuned."; echo "- Proactive USE flags configured and self-healing emerge performed."; echo "- XFCE Desktop Environment installed."; echo "- Hardware-aware kernel compiled and bootloader updated."; echo "- Interactive user account created."; echo "- Security foundation established (sudo, firewall)."; echo -e "\n${C_YELLOW}Recommended next step: REBOOT the system to apply all changes.${C_RESET}"; }

    # --- Main Execution Flow (Inside Chroot) ---
    # Source profile to get correct environment
    source /etc/profile
    export PS1="(chroot) ${PS1}"
    
    DRY_RUN=false; AUTO_MODE=false; FORCE_MODE=false
    # Shift away the --chrooted argument before parsing the rest
    shift
    ...
    log "Starting Stage 2: Main Installation..."
    
    preflight_checks
    command -v equery >/dev/null 2>&1 || emerge --onlydeps app-portage/gentoolkit
    ping -c1 -W2 gentoo.org >/dev/null 2>&1 || warn "Network is unreachable."
...
# ==============================================================================
main() {
    # Check for the special --chrooted argument.
    # If it's present, we are inside the chroot.
    if [[ "${1:-}" == "--chrooted" ]]; then
        run_installation_logic "$@"
    else
        # Otherwise, we are outside. Prepare the chroot and enter.
        prepare_and_enter_chroot "$@"
    fi
}
