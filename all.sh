...
# Version: 3.3 "Archon" (The Ruler)
#
# This version transcends simple automation, acting as an intelligent system architect.
# It introduces self-healing capabilities, hardware-aware kernel configuration,
# and a foundational security setup (sudo, firewall).
...
C_RED='\033[0;31m'
STEP_COUNT=0
TOTAL_STEPS=12 # Adjust this as you add/remove modules
...
load_config() {
  ...
  if [[ ! -f "$CONFIG_FILE" ]]; then
    ...
    cat > "$CONFIG_FILE" <<'EOF'
# Gentoo Autobuilder Configuration ("Archon" version)
...
# --- Emerge Settings ---
EMERGE_JOBS="$(nproc --all 2>/dev/null || echo 4)"
EOF
...
CURRENT_BACKUP=""; trap 'on_error $?' ERR
on_error() { local rc=${1:-1}; err "Script exited with code $rc. Attempting rollback..."; if [[ -n "$CURRENT_BACKUP" && -f "$CURRENT_BACKUP" ]]; then warn "Restoring from backup $CURRENT_BACKUP ..."; if ! $DRY_RUN; then tar -xzpf "$CURRENT_BACKUP" -C / || warn "Backup restoration failed."; fi; fi; err "See log for details: $LOG"; exit "$rc"; }
    ...
    while (( attempt <= 2 )); do
        if ! $DRY_RUN; then
            if emerge -j"${EMERGE_JOBS}" --load-average="${EMERGE_JOBS}" "${EMERGE_BASE_OPTS[@]}" "$pkg"; then
                command -v etc-update &>/dev/null && etc-update --automode -5
                log "Successfully installed $pkg."
                return 0
            fi
        else
            log "DRY-RUN: emerge $pkg"
            return 0
        fi

        warn "Failed to install $pkg on attempt $attempt."
        if (( attempt == 1 )); then
            warn "This might be a repository sync issue. Attempting 'emerge --sync' and retrying..."
            run_or_dry "emerge --sync"
        fi
        attempt=$((attempt + 1))
    done
    
    err "Could not install $pkg after 2 attempts."
    ask_confirm "Continue script execution?" || die "Execution aborted by user."
    return 1
}
update_conf_variable() { local conf_file="$1"; local var_name="$2"; local new_value="$3"; log "Updating $var_name in $conf_file"; if ! $DRY_RUN; then touch "$conf_file"; sed -i -E "/^${var_name}=/s/^/# (deactivated by builder) &/" "$conf_file"; echo -e "\n# Added by The Archon Autobuilder\n${var_name}=\"${new_value}\"" >> "$conf_file"; fi; }

# --- Main Script Modules ---
module_setup_localization() {
    step_log "Configuring System Localization"
    log "Setting timezone to ${SYSTEM_TIMEZONE}"
    run_or_dry "echo '${SYSTEM_TIMEZONE}' > /etc/timezone"
    run_or_dry "emerge --config sys-libs/timezone-data"
    
    log "Setting locale to ${SYSTEM_LOCALE}"
    if ! grep -q "${SYSTEM_LOCALE}" /etc/locale.gen; then
        run_or_dry "echo '${SYSTEM_LOCALE} UTF-8' >> /etc/locale.gen"
    fi
    run_or_dry "locale-gen"
    run_or_dry "eselect locale set '${SYSTEM_LOCALE}'"
    run_or_dry "env-update && source /etc/profile"
}
...
module_setup_package_use() { step_log "Configuring Proactive USE Flags (package.use)"; local package_use_dir="/etc/portage/package.use"; local use_file="$package_use_dir/99_autobuilder_flags"; log "Creating USE flag configuration at $use_file"; if ! $DRY_RUN; then mkdir -p "$package_use_dir"; cat > "$use_file" <<'EOF'; sys-auth/polkit elogind; x11-base/xorg-server elogind; xfce-base/xfce4-session elogind; x11-misc/lightdm gtk; media-libs/mesa X gallium vulkan; x11-libs/libnotify dbus; EOF; fi; }
module_install_desktop() { step_log "Installing XFCE Desktop Environment"; local desktop_packages=("x11-base/xorg-server" "x11-misc/lightdm" "x11-misc/lightdm-gtk-greeter" "xfce-base/xfce4-meta" "xfce-extra/xfce4-goodies" "media-libs/mesa" "app-admin/eselect" "sys-auth/elogind"); for pkg in "${desktop_packages[@]}"; do emerge_pkg "$pkg"; done; log "Configuring services for automatic startup..."; if ! $DRY_RUN; then mkdir -p /etc/lightdm/lightdm.conf.d; cat > /etc/lightdm/lightdm.conf.d/50-autobuilder.conf <<'EOF'; [Seat:*]; user-session=xfce; EOF; run_or_dry "rc-update add lightdm default"; run_or_dry "rc-update add elogind default"; fi; }
...
module_build_kernel() {
    step_log "Building Intelligent, Hardware-Aware Kernel"
    emerge_pkg "sys-kernel/gentoo-sources"; emerge_pkg "sys-kernel/genkernel"
    local latest_kernel_dir; latest_kernel_dir=$(ls -d /usr/src/linux-* | sort -V | tail -n 1); [[ -z "$latest_kernel_dir" ]] && die "Kernel sources not found in /usr/src."
    run_or_dry "ln -sfn '$latest_kernel_dir' /usr/src/linux"
    log "Symlink /usr/src/linux now points to $latest_kernel_dir"
    
    log "Generating kernel config with hardware auto-detection..."
    if ! $DRY_RUN; then
        pushd /usr/src/linux >/dev/null
        # 1. Get a safe base config
        if [[ -f /proc/config.gz ]]; then
            log "Using running kernel's config as a base."; zcat /proc/config.gz > .config
        else
            log "Creating default config."; make defconfig
        fi
        # 2. Auto-detect loaded modules for this specific hardware
        log "Running 'make localmodconfig' to tailor config to this hardware..."
        make localmodconfig
        popd >/dev/null
    fi
    # 3. Force-enable our required settings
    local built_in_symbols=(DEVTMPFS DEVTMPFS_MOUNT TMPFS EFI_STUB EFI_PARTITION XFS_FS EXT4_FS BLK_DEV_INITRD DRM_AMDGPU DRM_RADEON)
    if [[ -x "/usr/src/linux/scripts/config" ]]; then
        for sym in "${built_in_symbols[@]}"; do run_or_dry "'/usr/src/linux/scripts/config' --enable '$sym'"; done
    fi
    run_or_dry "make -C /usr/src/linux olddefconfig"

    if ask_confirm "Kernel configured. Begin compilation?"; then
        run_or_dry "make -C /usr/src/linux -j'${EMERGE_JOBS}'"
        run_or_dry "make -C /usr/src/linux modules_install"
        run_or_dry "make -C /usr/src/linux install"
        run_or_dry "genkernel --install initramfs"
    else
        warn "Kernel build skipped by user."
    fi
}
...
module_setup_security() {
    step_log "Establishing Security Foundation"
    log "Setting up sudo for the 'wheel' group..."
    emerge_pkg "app-admin/sudo"
    if ! $DRY_RUN; then
        echo '%wheel ALL=(ALL:ALL) ALL' > /etc/sudoers.d/wheel
    fi
    
    log "Setting up Uncomplicated Firewall (ufw)..."
    emerge_pkg "net-firewall/ufw"
    run_or_dry "ufw default deny incoming"
    run_or_dry "ufw default allow outgoing"
    run_or_dry "ufw allow ssh"
    run_or_dry "ufw enable"
    run_or_dry "rc-update add ufw default"
}
...
main() {
  ...
  log "Starting Gentoo Ultimate Autobuilder v3.3 'Archon': $(date --iso-8601=seconds)"
